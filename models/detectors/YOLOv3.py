...
class YOLOv3:
    ...
    def predict_single(self, image, color_mode='BGR'):
        detections,intermediate_features = self.predict(np.expand_dims(image.copy(), axis=0), color_mode=color_mode)
        return detections[0],intermediate_features
    ...
    def predict(self, images, color_mode='BGR'):
        ...
        with torch.no_grad():
            ...
            if len(images_rescaled) <= self.max_batch_size:
                detections,intermediate_features = self.model(images_rescaled)
                ...
                for i in range(0, len(images_rescaled), self.max_batch_size):
                    detections[i:i + self.max_batch_size],intermediate_feature = self.model(images_rescaled[i:i + self.max_batch_size]).detach()
                    intermediate_features +=intermediate_feature
            ...
            return detections,intermediate_features
